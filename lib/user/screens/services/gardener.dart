...
///
///  زهراء آل طلاق:
/// - تصميم أولي، عرض قائمة العمال لصفحة " تنسيق الحداق".
///
///  فاطمة اليامي:
...
class Gardener extends StatefulWidget {
  ...
  const Gardener({super.key, required this.userId, required this.userData});

  @override
  State<Gardener> createState() => _GardenerState();
}
...
class _GardenerState extends State<Gardener> {
  ...
  List<Widget> get _pages => [
    _buildGardenerContent(),
    MyBookingsPage(userId: widget.userId),
    ProfilePage(),
  ];

  Widget _buildGardenerContent() {
        ...
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance
                .collection('Workers')
                .where('type', isEqualTo: 'تنسيق حدائق')
