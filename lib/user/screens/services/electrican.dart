...
///
///  زهراء آل طلاق:
/// - تصميم أولي، عرض قائمة العمال لصفحة " الكهرباء".
///
///  فاطمة اليامي:
...
class _ElectriciansPageState extends State<ElectriciansPage> {
  ...
  List<Widget> get _pages => [
    _buildElectriciansContent(),
    MyBookingsPage(userId: widget.userId),
    ProfilePage(),
  ];

  Widget _buildElectriciansContent() {
        ...
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance
                .collection('Workers')
                .where('type', isEqualTo: 'كهربائي')
                .where('approved', isEqualTo: true)
                .snapshots(),
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                return Center(child: Text(tr('electricity.error')));
              }
              ...
              if (docs.isEmpty) {
                return Center(child: Text(tr('electricity.empty')));
              }
