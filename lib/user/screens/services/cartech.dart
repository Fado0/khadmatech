...
///
///  زهراء آل طلاق:
/// -تصميم أولي، عرض قائمة العمال لصفحة "تصليح السيارات ".
///
///  فاطمة اليامي:
...
class Cartech extends StatefulWidget {
  final String userId;
  final Map<String, dynamic> userData;

  const Cartech({super.key, required this.userId, required this.userData});

  @override
  State<Cartech> createState() => _CartechState();
}

class _CartechState extends State<Cartech> {
  ...
  List<Widget> get _pages => [
    _buildCarRepairContent(),
    MyBookingsPage(userId: widget.userId),
    ProfilePage(),
  ];

  Widget _buildCarRepairContent() {
    ...
    final iconColor = isDark
        ? const Color.fromARGB(255, 144, 174, 223)
        : const Color.fromARGB(255, 52, 85, 139);
        ...
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance
                .collection('Workers')
                .where('type', isEqualTo: 'اصلاح سيارات')
                .where('approved', isEqualTo: true)
                .snapshots(),
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                return Center(child: Text(tr('car_repair.error')));
              }
              ...
              if (docs.isEmpty) {
                return Center(child: Text(tr('car_repair.empty')));
              }
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      title: Text(
                        worker['name'] ?? tr('car_repair.no_name'),
                        style: TextStyle(
                          fontSize: 16,
                          fontWeight: FontWeight.bold,
                          color: textColor,
                      ...
                      subtitle: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(
                              '${tr('car_repair.service')}: ${worker['type']}',
                              style: TextStyle(
                                fontSize: 13,
                                color: subtitleColor,
                              ),
