...
///
///  زهراء آل طلاق:
/// - تصميم أولي، عرض قائمة العمال لصفحة " التنظيف".
///
///  فاطمة اليامي:
...
class Clean extends StatefulWidget {
  ...
  const Clean({super.key, required this.userId, required this.userData});
        ...
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance
                .collection('Workers')
                .where('type', isEqualTo: 'عاملة تنظيف')
                .where('approved', isEqualTo: true)
                .snapshots(),
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                return Center(child: Text(tr('clean.error')));
              }
              ...
              if (docs.isEmpty) {
                return Center(child: Text(tr('clean.empty')));
              }
