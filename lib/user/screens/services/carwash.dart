...
///
///  زهراء آل طلاق:
/// - تصميم أولي، عرض قائمة العمال لصفحة "غسيل السيارات ".
///
///  فاطمة اليامي:
...
class CarWash extends StatefulWidget {
  ...
  const CarWash({super.key, required this.userId, required this.userData});
...
class _CarWashState extends State<CarWash> {
  ...
  List<Widget> get _pages => [
    _buildCarWashContent(),
    MyBookingsPage(userId: widget.userId),
    ProfilePage(),
  ];

  Widget _buildCarWashContent() {
        ...
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance
                .collection('Workers')
                .where('type', isEqualTo: 'غسيل سيارات')
                .where('approved', isEqualTo: true)
                .snapshots(),
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                return Center(child: Text(tr('car_wash.error')));
              }
              ...
              if (docs.isEmpty) {
                return Center(child: Text(tr('car_wash.empty')));
              }
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      title: Text(
                        worker['name'] ?? tr('car_wash.no_name'),
                        style: TextStyle(
                          fontSize: 16,
                          fontWeight: FontWeight.bold,
                          color: textColor,
                      ...
                      subtitle: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(
                              '${tr('car_wash.service')}: ${worker['type']}',
                              style: TextStyle(
                                fontSize: 13,
                                color: subtitleColor,
                              ),
