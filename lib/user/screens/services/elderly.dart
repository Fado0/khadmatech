...
///
///  زهراء آل طلاق:
/// - تصميم أولي، عرض قائمة العمال لصفحة " رعاية كبار السن".
///
///  فاطمة اليامي:
...
class Elderly extends StatefulWidget {
  final String userId;
  final Map<String, dynamic> userData;

  const Elderly({super.key, required this.userId, required this.userData});

  @override
  State<Elderly> createState() => _ElderlyState();
}

class _ElderlyState extends State<Elderly> {
  ...
  List<Widget> get _pages => [
    _buildElderlyContent(),
    MyBookingsPage(userId: widget.userId),
    ProfilePage(),
  ];

  Widget _buildElderlyContent() {
        ...
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance
                .collection('Workers')
                .where('type', isEqualTo: 'رعاية كبار السن')
                .where('approved', isEqualTo: true)
                .snapshots(),
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                return Center(child: Text(tr('elderly.error')));
              }
              ...
              if (docs.isEmpty) {
                return Center(child: Text(tr('elderly.empty')));
              }
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      title: Text(
                        worker['name'] ?? tr('elderly.no_name'),
                        style: TextStyle(
                          fontSize: 16,
                          fontWeight: FontWeight.bold,
                          color: textColor,
                      ...
                      subtitle: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(
                              '${tr('elderly.service')}: ${worker['type']}',
                              style: TextStyle(
                                fontSize: 13,
                                color: subtitleColor,
                              ),
  ...
  Widget build(BuildContext context) {
    return Directionality(
      textDirection: Directionality.of(context), // ✅ كما طلبتِ
      child: Scaffold(
